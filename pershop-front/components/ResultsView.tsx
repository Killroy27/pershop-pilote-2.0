...
export default function ResultsView({ results, onRetry }: ResultsViewProps) {
  ...
  return (
    <>
      ...
      <div className={`space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 ${selectedShopper ? 'blur-sm scale-[0.98] opacity-50 pointer-events-none transition-all' : 'transition-all'}`}>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {shoppers_found.map((shopper: any, i: number) => (
            <Card 
              ...
              className="group cursor-pointer hover:border-amber-500/50 transition-all duration-300 hover:-translate-y-2 overflow-hidden relative bg-secondary/20 hover:shadow-[0_0_30px_-10px_rgba(251,191,36,0.15)]"
            >
              {i === 0 && (
              ...
              <CardContent className="-mt-12 relative z-10">
                <div className="flex justify-center mb-4">
                  <div className="h-24 w-24 rounded-full bg-neutral-200 border-4 border-card flex items-center justify-center text-3xl font-bold text-neutral-500 group-hover:border-amber-500/50 transition-colors">
                    {shopper.shopper_name.charAt(0)}
                  </div>
                </div>
                
                <div className="text-center space-y-2 mb-6">
                  <h3 className="text-xl font-bold font-serif group-hover:text-primary transition-colors">{shopper.shopper_name}</h3>
